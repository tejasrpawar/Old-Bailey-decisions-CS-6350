...
def logistic_regression(X, y, lr_init, sigma_sq, epochs):
    ...
    for epoch in tqdm(range(1, epochs + 1)):
        ...
        for i in range(len(y)):
            ...
            sigma_term = 2 * w / sigma_sq
            z = yi * np.dot(w, xi)

            #handling overflow
            if z > 0:
                exp_term = np.exp(-z) / (1 + np.exp(-z))
            else:
                exp_term = 1 / (1 + np.exp(z))

            gradient = sigma_term - exp_term * yi * xi
            w = w - lr * gradient
    ...
    for lr in lr_candidates:
        for t in tradeoff_candidates:
            ...
            for num in tqdm(range(folds)):
                ...
                w = logistic_regression(x_train_cv, label_train_cv, lr, t, epochs)
            ...
            print(f"lr: {lr} | tradeoff: {t} | avg acc {np.round(avg_acc, 3)}")
            if avg_acc > best_acc:
                best_acc = avg_acc
                best_lr = lr
                best_tradeoff = t
...
"""
# best_learning_rate, best_sigma_sq_value = cross_validation(x_train_tfidf_features, y_train_tfidf_labels, [0.1, 0.01, 0.001], [10, 1, 1000, 10000, 100], 10, 5)

# Train Logistic regression
# weights = logistic_regression(x_train_tfidf_features, y_train_tfidf_labels, best_learning_rate, best_sigma_sq_value, 10)
# After running cross validation and finding the best hyperparameter.
weights = logistic_regression(x_train_tfidf_features, y_train_tfidf_labels, 0.1, 10000, 15)
...
print("Logistic Regression accuracy on train data tfidf:", accuracy_train)

# Test
predictions_test = predict(x_test_tfidf, weights)
accuracy_test = calculate_accuracy(predictions_test, labels_test_tfidf)
print("Logistic Regression accuracy on test data tfidf:", accuracy_test)

predictions_eval = predict(x_eval_tfidf, weights)

with open('tfidf_lr.csv', 'w', newline='') as file:
...
do not want to run it again and again because it took time.
"""
# best_learning_rate, best_sigma_sq_value = cross_validation(x_train_bow_features, y_train_bow_labels, [1, 0.1, 0.01], [10, 1, 1000, 10000, 100], 10, 5)

# Train Logistic Regression
# weights = logistic_regression(x_train_glove_features, y_train_glove_labels, best_learning_rate, best_sigma_sq_value, 10)
# After running cross validation and finding the best hyperparameter.
weights = logistic_regression(x_train_bow_features, y_train_bow_labels, 0.1, 10000, 15)
...
print("Logistic Regression accuracy on train data bow:", accuracy_train)

# Test
predictions_test = predict(x_test_bow, weights)
accuracy_test = calculate_accuracy(predictions_test, labels_test_bow)
print("Logistic Regression accuracy on test data bow:", accuracy_test)

predictions_eval = predict(x_eval_bow, weights)

with open('bow_lr.csv', 'w', newline='') as file:
    writer = csv.writer(file)
    field = ["example_id", "label"]
...
do not want to run it again and again because it took time.
"""
# best_learning_rate, best_sigma_sq_value = cross_validation(x_train_glove_features, y_train_glove_labels, [1, 0.1, 0.01], [10, 1, 1000, 10000, 100], 10, 5)

# Train Logistic Regression
# weights = logistic_regression(x_train_glove_features, y_train_glove_labels, best_learning_rate, best_sigma_sq_value, 10)
# After running cross validation and finding the best hyperparameter.
weights = logistic_regression(x_train_glove_features, y_train_glove_labels, 0.1, 10000, 15)
...
print("Logistic Regression accuracy on train data glove:", accuracy_train)

# Test
predictions_test = predict(x_test_glove, weights)
accuracy_test = calculate_accuracy(predictions_test, labels_test_glove)
print("Logistic Regression accuracy on test data glove:", accuracy_test)

predictions_eval = predict(x_eval_glove, weights)

with open('glove_lr.csv', 'w', newline='') as file:
    writer = csv.writer(file)
    field = ["example_id", "label"]
